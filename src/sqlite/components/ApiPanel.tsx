 ...
 * agent is querying the open database, the interesting thing is watching the
 * calls arrive — which query, how many rows, how long — while the data tab next
 * to it shows the same database.
 */

interface ApiPanelProps {
  /** Shown in the examples, so a copied curl line targets a real table. */
...
export const ApiPanel: React.FC<ApiPanelProps> = ({ sampleTable }) => {
  ...
  const examples = useMemo(
    () => [
      {
        title: 'Schema lesen',
        command: `curl -s ${baseUrl}/api/v1/sqlite/schema -H "Authorization: Bearer ${token}"`,
      },
      {
        title: 'SQL ausführen',
        command:
          `curl -s ${baseUrl}/api/v1/sqlite/sql -H "Authorization: Bearer ${token}" \\\n` +
          `  -H "Content-Type: application/json" \\\n` +
          `  -d '{"sql":"SELECT * FROM ${sampleTable ?? 'sqlite_master'} LIMIT 5"}'`,
      },
      {
        title: 'Zeilen einer Tabelle',
        command:
          `curl -s "${baseUrl}/api/v1/sqlite/tables/${sampleTable ?? 'meine_tabelle'}?limit=20&offset=0" \\\n` +
          `  -H "Authorization: Bearer ${token}"`,
      },
    ],
    [baseUrl, token, sampleTable]
  );

  const endpoints: Array<[string, string]> = [
    ['GET  /api/v1/sqlite/health', 'Läuft die App, welche Datenbank ist offen'],
    ['GET  /api/v1/sqlite/schema', 'Alle Tabellen mit Spalten, Schlüsseln, Indizes'],
    ['GET  /api/v1/sqlite/tables', 'Kurzliste der Tabellen und Views'],
    ['GET  /api/v1/sqlite/tables/:name', 'Zeilen, mit limit, offset, search, orderBy, order'],
    ['GET  /api/v1/sqlite/tables/:name/schema', 'Nur die Struktur einer Tabelle'],
    ['POST /api/v1/sqlite/sql', '{ sql, limit?, format? } — beliebiges SQL'],
    ['POST /api/v1/sqlite/open', '{ path } — Datei in die laufende Sitzung laden'],
    ['GET  /api/v1/sqlite/integrity', 'PRAGMA integrity_check'],
  ];
      ...
      <div className="max-w-4xl mx-auto space-y-5">
        <div>
          ...
          <p className="text-apple-text-secondary text-xs mt-0.5">
            Ein HTTP-Zugang auf 127.0.0.1 zur Datenbank, die gerade offen ist — für andere
        ...
        <div className="p-5 rounded-xl bg-[#141722]/80 border border-white/[0.08] shadow-apple-sm space-y-1">
          ...
          <Row label="Schreibzugriff">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-apple-text-secondary">
                INSERT, UPDATE, DELETE und DDL über die API erlauben
              </span>
            </label>
          </Row>
          ...
          <p className="text-[11px] text-apple-text-muted leading-relaxed pt-2 border-t border-white/[0.06] mt-2">
            ...
            Token. Änderungen betreffen wie überall in der App nur die Kopie im Speicher — die
            Datei auf der Platte bleibt unberührt, bis du exportierst.
          </p>
        </div>
              ...
              {log.map((entry) => (
                <div key={entry.id} className="px-5 py-2 flex items-start gap-3 hover:bg-white/[0.02]">
                  ...
                  <span className="font-mono text-[11px] text-apple-text-tertiary shrink-0 tabular-nums">
                    {entry.rowCount === null ? '—' : `${entry.rowCount} Z.`}
                  </span>
